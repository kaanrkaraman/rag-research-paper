...
\item {\bf Declaration of LLM usage}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: LLMs appear in three distinct roles in this work, all standard but each method-relevant. (i) As components of evaluated retrieval methods: GPT-4.1-mini drives HyDE hypothetical-passage generation, Multi-Query reformulation, CRAG relevance evaluation and rewriting, and Contextual Retrieval per-document context generation (Section~\ref{sec:methods}; prompts in Appendix~\ref{app:prompts}). (ii) As the answer generator for the end-to-end Number Match evaluation: GPT-4.1-mini at temperature 0 (Section~\ref{sec:generation}; prompt in Appendix~\ref{app:prompts}). (iii) As a labelling tool for the failure taxonomy: GPT-5.4 was used to classify $n{=}100$ sampled hybrid-RRF retrieval failures into five categories (Section~\ref{sec:errors}). Model names, temperatures, prompts, and seeds are reported throughout. No LLM was used to generate experimental results or claims in the paper.
