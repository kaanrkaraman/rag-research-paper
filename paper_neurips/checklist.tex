...
\item {\bf Experiments compute resources}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:setup} describes the compute setup. BM25, FAISS, and the closed-stack experiments (\texttt{text-embedding-3-large} embedding plus Cohere Rerank) run locally on Apple Silicon with Azure AI Foundry serving the API endpoints; no GPU training is required. The open-weight robustness panel (BGE-M3 dense, BGE-M3 hybrid, BGE-M3 hybrid + \texttt{bge-reranker-v2-m3}) was evaluated on a single NVIDIA datacenter GPU (H100 80~GB or equivalent, $\geq$40~GB sufficient); the documented end-to-end wall-clock is approximately 4~hours, dominated by the cross-encoder reranker pass over the full 23{,}088-query test set.
